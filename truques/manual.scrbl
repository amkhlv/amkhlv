...
(autolist [#:exts list-of-extensions (listof symbol?) '(pdf)]
          ...
          [#:output f (-> path-string? (or/c (listof any/c))) (lambda (p)   `(,(hyperlink (path->string (path->complete-path (build-path (get-bystro-scrbl-name) p))) (path->string p))))]
          )
table?
